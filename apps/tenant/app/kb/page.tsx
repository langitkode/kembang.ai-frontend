...
export default function KBOrchestratorPage() {
  ...
  async function fetchDocs() {
    ...
    console.log("KB: Fetching documents with token");
    try {
      setLoading(true);
      // TODO: Add tenant filtering to API when backend supports it
      // const data = tenantId ? await api.getTenantDocuments(tenantId) : await api.getDocuments();
      ...
      <header className="flex justify-between items-end border-b pb-6">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-sm">
            {tenantId
              ? `Managing documents for tenant: ${tenantId}`
